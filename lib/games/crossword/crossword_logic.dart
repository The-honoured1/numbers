...
class CrosswordLogic {
  ...
  CrosswordData generate(int level) {
    ...
    while (true) {
      ...
      int r1 = _evaluate(values[0], ops[0], values[1], ops[1], values[2]);
      int r2 = _evaluate(values[3], ops[5], values[4], ops[6], values[5]);
      int r3 = _evaluate(values[6], ops[10], values[7], ops[11], values[8]);

      // Col results with BODMAS
      int r4 = _evaluate(values[0], ops[2], values[3], ops[7], values[6]);
      int r5 = _evaluate(values[1], ops[3], values[4], ops[8], values[7]);
      int r6 = _evaluate(values[2], ops[4], values[5], ops[9], values[8]);
  ...
  int _evaluate(int a, String op1, int b, String op2, int c) {
    if (op2 == '×' && op1 != '×') {
      int sub = b * c;
